...
function animatedCounters() {
  ...
  function initAndAnimateCounters(container) {
    ...
    counters.forEach((counter) => {
      if (!counter.dataset.digitsCounterTarget) {
        const originalValue = counter.innerHTML;
        counter.dataset.digitsCounterTarget = originalValue;
        counter.innerHTML = "0";
  ...
  function handleWatcherCallback(e) {
    ...
    const targetElement = e.detail.entry.target;
...
function createFetchParam(eventTarget) {
  ...
  if (eventTarget.matches("[data-section-id]")) {
    ...
    result.append("sectionId", sectionId);
  }
  return result;
}
